...
func (c *QQClient) buildLoginPacket() (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(c.Uin, 0x0810, crypto.ECDH, c.RandomKey, func(w *binary.Writer) {
		...
		w.Write(tlv.T1(uint32(c.Uin), c.SystemDeviceInfo.IpAddress))
		w.Write(tlv.T106(uint32(c.Uin), 0, c.version.AppId, c.version.SSOVersion, c.PasswordMd5, true, c.SystemDeviceInfo.Guid, c.SystemDeviceInfo.TgtgtKey, 0))
		...
		w.Write(tlv.T144(
			utils.S2B(c.SystemDeviceInfo.IMEI),
			c.SystemDeviceInfo.GenDeviceInfoData(),
			utils.S2B(c.SystemDeviceInfo.OSType),
			utils.S2B(c.SystemDeviceInfo.Version.Release),
			utils.S2B(c.SystemDeviceInfo.SimInfo),
			utils.S2B(c.SystemDeviceInfo.APN),
			false, true, false, tlv.GuidFlag(),
			utils.S2B(c.SystemDeviceInfo.Model),
			c.SystemDeviceInfo.Guid,
			utils.S2B(c.SystemDeviceInfo.Brand),
			c.SystemDeviceInfo.TgtgtKey,
		))

		w.Write(tlv.T145(c.SystemDeviceInfo.Guid))
		...
		*/
		...
		w.Write(tlv.T141(utils.S2B(c.SystemDeviceInfo.SimInfo), utils.S2B(c.SystemDeviceInfo.APN)))
		w.Write(tlv.T8(2052))
		...
		w.Write(tlv.T187(utils.S2B(c.SystemDeviceInfo.MacAddress)))
		w.Write(tlv.T188(utils.S2B(c.SystemDeviceInfo.AndroidId)))
		if len(c.SystemDeviceInfo.IMSIMd5) != 0 {
			w.Write(tlv.T194(c.SystemDeviceInfo.IMSIMd5))
		}
		if c.AllowSlider {
			w.Write(tlv.T191(0x82))
		}
		if len(c.SystemDeviceInfo.WifiBSSID) != 0 && len(c.SystemDeviceInfo.WifiSSID) != 0 {
			w.Write(tlv.T202(utils.S2B(c.SystemDeviceInfo.WifiBSSID), utils.S2B(c.SystemDeviceInfo.WifiSSID)))
		}
	...
	sso := packets.BuildSsoPacket(seq, c.version.AppId, c.version.SubAppId, "wtlogin.login", c.SystemDeviceInfo.IMEI, []byte{}, c.OutGoingPacketSessionId, req, c.ksid)
...
func (c *QQClient) buildQRCodeFetchRequestPacket() (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(0, 0x812, crypto.ECDH, c.RandomKey, func(w *binary.Writer) {
		...
		w.Write(packets.BuildCode2DRequestPacket(0, 0, 0x31, func(w *binary.Writer) {
			...
			w.WriteUInt16(6)
			w.Write(tlv.T16(watch.SSOVersion, 16, watch.AppId, c.SystemDeviceInfo.Guid, []byte(watch.ApkId), []byte(watch.SortVersionName), watch.ApkSign))
			w.Write(tlv.T1B(0, 0, 3, 4, 72, 2, 2))
			w.Write(tlv.T1D(watch.MiscBitmap))
			w.Write(tlv.T1F(false, utils.S2B(c.SystemDeviceInfo.OSType), []byte("7.1.2"), []byte("China Mobile GSM"), utils.S2B(c.SystemDeviceInfo.APN), 2))
			w.Write(tlv.T33(c.SystemDeviceInfo.Guid))
			w.Write(tlv.T35(8))
		}))
	})
	sso := packets.BuildSsoPacket(seq, watch.AppId, c.version.SubAppId, "wtlogin.trans_emp", c.SystemDeviceInfo.IMEI, []byte{}, c.OutGoingPacketSessionId, req, c.ksid)
...
func (c *QQClient) buildRequestTgtgtNopicsigPacket() (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(c.Uin, 0x0810, crypto.NewEncryptSession(c.sigInfo.t133), c.sigInfo.wtSessionTicketKey, func(w *binary.Writer) {
		...
		w.Write(tlv.T107(0))
		w.Write(tlv.T144(
			utils.S2B(c.SystemDeviceInfo.AndroidId),
			c.SystemDeviceInfo.GenDeviceInfoData(),
			utils.S2B(c.SystemDeviceInfo.OSType),
			utils.S2B(c.SystemDeviceInfo.Version.Release),
			utils.S2B(c.SystemDeviceInfo.SimInfo),
		...
		w.Write(tlv.T147(16, []byte(c.version.SortVersionName), c.version.ApkSign))
		w.Write(tlv.T177(c.version.BuildTime, c.version.SdkVersion))
		w.Write(tlv.T400(c.g, c.Uin, c.SystemDeviceInfo.Guid, c.dpwd, 1, 16, c.randSeed))
		w.Write(tlv.T187(utils.S2B(c.SystemDeviceInfo.MacAddress)))
		w.Write(tlv.T188(utils.S2B(c.SystemDeviceInfo.AndroidId)))
		w.Write(tlv.T194(c.SystemDeviceInfo.IMSIMd5))
		w.Write(tlv.T202(utils.S2B(c.SystemDeviceInfo.WifiBSSID), utils.S2B(c.SystemDeviceInfo.WifiSSID)))
...
func (c *QQClient) buildRequestChangeSigPacket() (uint16, []byte) {
	...
	req := packets.BuildOicqRequestPacket(c.Uin, 0x0810, crypto.ECDH, c.RandomKey, func(w *binary.Writer) {
		...
		w.Write(tlv.T116(c.version.MiscBitmap, c.version.SubSigmap))
		w.Write(tlv.T108(c.SystemDeviceInfo.IMEI))
		...
		// w.Write(tlv.T202(c.SystemDeviceInfo.WifiBSSID, c.SystemDeviceInfo.WifiSSID))
	})
	sso := packets.BuildSsoPacket(seq, c.version.AppId, c.version.SubAppId, "wtlogin.exchange_emp", c.SystemDeviceInfo.IMEI, c.sigInfo.tgt, c.OutGoingPacketSessionId, req, c.ksid)
	packet := packets.BuildLoginPacket(c.Uin, 2, make([]byte, 16), sso, []byte{})
	return seq, packet
}
...
func (c *QQClient) buildClientRegisterPacket() (uint16, []byte) {
	...
	svc := &jce.SvcReqRegister{
		...
		KickPC:       0,
		KickWeak:     0,
		IOSVersion:   int64(c.SystemDeviceInfo.Version.Sdk),
		NetType:      1,
		RegType:      0,
		Guid:         c.SystemDeviceInfo.Guid,
		IsSetStatus:  0,
		LocaleId:     2052,
		DevName:      string(c.SystemDeviceInfo.Model),
		DevType:      string(c.SystemDeviceInfo.Model),
		OSVer:        string(c.SystemDeviceInfo.Version.Release),
		OpenPush:     1,
		LargeSeq:     1551,
		OldSSOIp:     0,
		...
		ChannelNo:    "",
		CPID:         0,
		VendorName:   string(c.SystemDeviceInfo.VendorName),
		VendorOSName: string(c.SystemDeviceInfo.VendorOSName),
	...
	sso := packets.BuildSsoPacket(seq, c.version.AppId, c.version.SubAppId, "StatSvc.register", c.SystemDeviceInfo.IMEI, c.sigInfo.tgt, c.OutGoingPacketSessionId, pkt.ToBytes(), c.ksid)
...
func (c *QQClient) buildAppInfoRequestPacket(id string) (uint16, []byte) {
	...
	body := &qweb.QWebReq{
		...
		DeviceInfo: fmt.Sprintf("i=865166025905020&imsi=460002478794049&mac=02:00:00:00:00:00&m=%v&o=7.1.2&a=25&sc=1&sd=0&p=900*1600&f=nubia&mm=3479&cf=2407&cc=4&aid=086bbf84a7d5fbb3&qimei=865166023450458&sharpP=1&n=wifi", string(c.SystemDeviceInfo.Model)),
		BusiBuff:   b,
