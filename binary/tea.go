...
func (t TEA) Encrypt(src []byte) (dst []byte) {
	...
	for i := 0; i < len(dst); i += 8 {
		...
		iv1 = enctea(t0, t1, t2, t3, holder) ^ iv2
		iv2 = holder
...
func (t TEA) Decrypt(data []byte) []byte {
	...
	for i := 0; i < len(dst); i += 8 {
		...
		iv2 = dectea(t0, t1, t2, t3, iv1^iv2)
...
//go:nosplit
func enctea(t0, t1, t2, t3 uint32, n uint64) uint64 {
	...
	v0 += ((v1 << 4) + t0) ^ (v1 + 0x08d12e65) ^ ((v1 >> 5) + t1)
	v1 += ((v0 << 4) + t2) ^ (v0 + 0x08d12e65) ^ ((v0 >> 5) + t3)
...
// 每次8字节
//go:nosplit
func dectea(t0, t1, t2, t3 uint32, n uint64) uint64 {
	v0, v1 := uint32(n>>32), uint32(n)
